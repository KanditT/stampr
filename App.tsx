...
const App: React.FC = () => {
  ...
  const [selectedStampDetail, setSelectedStampDetail] = useState<Stamp | null>(
    null,
  );
  ...
  const selectSymbol = async (symbol: string) => {
    ...
    const randomStyle =
      STAMP_STYLES[Math.floor(Math.random() * STAMP_STYLES.length)];
  ...
  const confirmPlacement = () => {
    ...
    setState((prev) => ({
      ...
      collectedStamps: {
        ...
        [finalStamp.locationId]: finalStamp,
      },
    }));
  ...
  const resetJournal = () => {
    if (
      window.confirm(
        "คุณต้องการลบข้อมูลทั้งหมดและเริ่มต้นใหม่ใช่หรือไม่? (ข้อมูลจะหายไปถาวร)",
      )
    ) {
  ...
  return (
    <div className="min-h-screen bg-dreamy-bg pb-44 select-none">
      <header className="sticky top-0 z-40 bg-dreamy-bg/90 backdrop-blur-md border-b border-dreamy-border px-6 py-4 flex justify-between items-center no-print">
        <div>
          <h1 className="text-xl font-display font-bold text-dreamy-text">
            Heart of Music
          </h1>
          <p className="text-sm uppercase tracking-widest font-display text-dreamy-text-secondary font-medium">
            {Object.keys(state.collectedStamps).length} / {LOCATIONS.length}{" "}
            Rooms
          </p>
        </div>
        <div className="flex space-x-2">
          <button
            onClick={downloadSymphony}
            className="p-3 bg-dreamy-surface rounded-full shadow-dreamy text-dreamy-accent active:scale-90 transition-transform"
          >
            <svg
          ...
          <button
            onClick={() => setView(view === "book" ? "creator" : "book")}
            className="p-3 bg-dreamy-accent text-white rounded-full"
          >
            {view === "book" ? "⚙️" : "𝄞"}
          </button>
        </div>
      ...
      <main className="p-4 max-w-4xl mx-auto">
        {view === "book" ? (
          <div className="space-y-12">
            <div
              ref={manuscriptRef}
              className="manuscript-paper relative overflow-hidden"
            >
              <div className="mb-12 border-b-2 border-dreamy-border pb-4 flex justify-between items-end bg-linear-to-r from-dreamy-surface via-[#FFF0E6] to-dreamy-surface rounded-xl p-6">
                <div>
                  <h2 className="text-3xl font-display font-bold text-dreamy-text">
                    My Museum Composition
                  </h2>
                  <p className="text-sm font-display uppercase tracking-widest text-dreamy-text-secondary font-medium">
                    Composition Journal
                  </p>
                </div>
                <span className="font-display text-sm text-dreamy-text-secondary font-medium">
                  {new Date().toLocaleDateString("th-TH")}
                </span>
              </div>

              <div className="staff-container relative">
                <span className="absolute -left-2 top-1/2 -translate-y-1/2 text-5xl font-display text-dreamy-accent z-0">
                  𝄞
                </span>
              </div>

              <div className="staff-container relative mt-16">
                <span className="absolute -left-2 top-1/2 -translate-y-1/2 text-5xl font-display text-dreamy-accent z-0 opacity-40">
                  𝄞
                </span>
              </div>
                ...
                (loc) =>
                  state.collectedStamps[loc.id] && (
                    <StampItem
                      ...
                      onClick={() =>
                        setSelectedStampDetail(state.collectedStamps[loc.id])
                      }
                    />
                  ),
              )}
              ...
              <div className="mt-20 text-center">
                <p className="font-thai text-lg text-dreamy-text-secondary italic">
                  "หัวใจแห่งเสียงดนตรีจะ
                  <br />
                  บันทึกอยู่ในทุกตัวโน้ตที่คุณวาง"
                </p>
              </div>
            </div>
            ...
            {!pendingStamp && (
              <section className="px-4">
                <h3 className="font-thai font-bold text-dreamy-text mb-4 opacity-60">
                  ห้องที่รอการค้นพบ
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {LOCATIONS.filter((l) => !state.collectedStamps[l.id]).map(
                    (loc) => (
                      <div
                        key={loc.id}
                        className="p-4 bg-dreamy-surface/60 border border-dreamy-border border-dashed rounded-2xl flex items-center space-x-4"
                      >
                        <div className="w-8 h-8 bg-dreamy-border/30 rounded-full flex items-center justify-center font-display text-dreamy-accent/50">
                          ?
                        </div>
                        <div className="flex-1">
                          <p className="text-base font-thai font-semibold text-dreamy-text">
                            {loc.name}
                          </p>
                          <p className="text-base font-thai text-dreamy-text-secondary italic">
                            "{loc.hint}"
                          </p>
                        </div>
                      </div>
          ...
          <div className="space-y-6">
            <div className="bg-red-50/50 border border-red-200/60 p-6 rounded-3xl mb-8">
              <h3 className="text-red-800 font-bold mb-2 font-thai">
                ตั้งค่าสมุดบันทึก
              </h3>
              <p className="text-red-700 text-sm mb-4 font-thai">
                สำหรับการใช้เครื่องร่วมกัน หรือเริ่มบทเพลงใหม่
              </p>
              <button
        ...
        {pendingStamp?.isPlacing ? (
          ...
          view === "book" && (
            ...
            >
              ...
              <span className="font-thai font-bold text-lg tracking-wide">
                สแกนโค้ดห้องนิทรรศการ
              </span>
            </button>
          )
        )}
      ...
      {discoveredRoom && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-dreamy-overlay backdrop-blur-xl animate-in fade-in">
          <div className="bg-dreamy-surface w-full max-w-md rounded-[3rem] p-10 text-center shadow-dreamy-lg">
            <h2 className="text-lg font-display uppercase tracking-[0.3em] text-dreamy-text-secondary mb-2">
              Discovery
            </h2>
            <h3 className="text-4xl font-thai font-bold text-dreamy-text mb-4">
              {discoveredRoom.name}
            </h3>
            <p className="text-dreamy-text-secondary font-thai text-xl mb-12">
              เลือก 1 ชิ้นส่วนดนตรีที่คุณประทับใจที่สุดจากห้องนี้
            </p>
                ...
                >
                  {isImage(sym) ? (
                    <img
                      src={sym}
                      alt="Option"
                      className="w-full h-full object-contain pointer-events-none group-hover:invert transition-all"
                    />
                  ) : (
                    <span className="text-5xl text-dreamy-text group-hover:text-white">
                      {sym}
                    </span>
                  )}
                </button>
            ...
            <button
              onClick={() => setDiscoveredRoom(null)}
              className="text-dreamy-text-secondary font-display uppercase text-sm tracking-widest font-bold"
            >
              Cancel
            </button>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-dreamy-overlay-light backdrop-blur-sm no-print">
          <div className="bg-dreamy-surface w-full max-w-sm rounded-[3rem] p-10 relative border-2 border-dreamy-border animate-in zoom-in duration-300 shadow-dreamy-lg">
            <button
              ...
              className="absolute top-6 right-6 text-dreamy-text-secondary"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-6 w-6"
            ...
            <div className="text-center">
              <div className="w-40 h-40 mx-auto mb-8 flex items-center justify-center">
                {isImage(selectedStampDetail.symbol) ? (
                  <img
                    src={selectedStampDetail.symbol}
                    alt="Symbol"
                    className="max-w-full max-h-full object-contain"
                  />
                ) : (
                  <span className="text-8xl text-dreamy-accent">
                    {selectedStampDetail.symbol}
                  </span>
                )}
              </div>
              <h3 className="text-3xl font-thai font-bold mb-1">
                {selectedStampDetail.locationName}
              </h3>
              <p className="text-sm font-display uppercase tracking-widest text-dreamy-text-secondary mb-6">
                {selectedStampDetail.style} Edition
              </p>
              <p className="text-dreamy-text-secondary font-thai leading-relaxed italic text-lg">
                "{selectedStampDetail.lore}"
              </p>
            </div>
            <button
              onClick={() => setSelectedStampDetail(null)}
              className="mt-10 w-full py-4 bg-dreamy-border/30 rounded-2xl font-thai font-bold text-dreamy-text-secondary uppercase text-sm tracking-widest"
            >
              ปิดหน้าต่าง
            </button>
          </div>
